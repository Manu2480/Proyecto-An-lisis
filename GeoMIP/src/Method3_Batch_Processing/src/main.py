...
def iniciar():
    ...
    for sample_dir in sample_dirs:
        ...
        for sample_file in sample_dir.glob("N*.csv"):
            match = pattern.match(sample_file.name)
            if match:
                todos_los_csvs.append((sample_file, int(match.group(1))))

    if not todos_los_csvs:
        raise FileNotFoundError("No hay archivos de muestras TPM disponibles en data/samples ni .samples.")

    archivos_unicos = {f.name: (f, n) for f, n in todos_los_csvs}

    for nombre, (archivo_path, n_bits) in archivos_unicos.items():
        print(f"\n--- Procesando {nombre} (N={n_bits}) ---")
        ruta_salida = Path(
            os.getenv(
                "GEOMIP_OUTPUT_XLSX",
                str(GEOMIP_ROOT / "results" / f"resultados_Geometric_{nombre.replace('.csv', '')}.xlsx"),
            )
        )
        estado_inicio = "1" + ("0" * (n_bits - 1))
        
        ejecutar_desde_excel(ruta_entrada, ruta_salida, archivo_path, estado_inicio, inicio=0, cantidad=50)